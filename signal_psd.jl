...
# ╔═╡ 2e8e4bba-b6c5-11ec-3904-c3a1df846d95
begin
	using NPZ
	using DataFrames
	using CairoMakie
...
# ╔═╡ 535f84ad-3a6a-4ef9-bad2-d9906f093e8d
md"""
# Signal Spectral analysis
"""

# ╔═╡ b2f5c722-2bbe-4b3e-a00b-5f70f7e795e3
begin
	x = 40.0
	y = 40.0
	file_string = "../Phi_x_$(x)_y_$(y)_z_None.npz.npy"
end

# ╔═╡ ae772240-c211-464d-a7a4-804404971da0
begin
	field_data = DataFrame(
		npzread(file_string),
		[:iterations, :time, :data]
	)
end

# ╔═╡ 688b5f45-1ea7-4a03-aa31-dba6af6ed49c
begin
	# Computing the periodogram
	data_dt = field_data.time[2] - field_data.time[1]
	data_fs = 1 / data_dt
	p = periodogram(field_data.data, fs=data_fs)

	multipole_label = "\\Phi(t, $(x), $(y), 0)"
	...
	lines!(field_axis, field_data.time, field_data.data)
	lines!(psd_axis, p.freq, p.power)

	fig
end

# ╔═╡ 1ab1925c-925f-439d-9b94-9d62cdd03ba5
begin
