...
# ╔═╡ b8c661bc-a3c1-11ec-09b9-7d13a007775e
md"""
# Multipole data settings:
"""

# ╔═╡ 80c47a29-f881-46cc-96cb-a81b42eb5e19
begin
	root_dir = "/home/lucas/EinsteinToolkit/Cactus/exe/"
	
	creation_date = "07_04_2022_20:35:54"

	function_name = "KG_Phi"
...
# ╔═╡ 4e413cd9-2902-4a60-a28d-1bda6ee4d858
namestring = root_dir * function_name * "_" * string(l) * string(m) * "_r" * string(r) * "_" * creation_date * ".ascii"

# ╔═╡ 4ab5944c-1c78-42fe-9900-ad188d551659
file_data = CSV.read(namestring, delim=" ", ignorerepeated=true, types=Float64, header=["t", "Re", "Im"], DataFrame)

# ╔═╡ ec4b25a9-3e71-4f1f-b4e0-25cdc05adbae
time_step = file_data."t"[2]

# ╔═╡ 5f7a965f-c19a-44e0-8412-ab53299de10a
md"""
# Data plots
"""
...
delim_plot_start_idx = Int64(ceil(delim_plot_start_time/time_step)) + 1

# ╔═╡ 7da7fcf2-b7ef-4d90-812c-0402d2f28b48
delim_plot_end_idx = Int64(ceil(delim_plot_plot_end_time/time_step)) + 1

# ╔═╡ cb696b08-2439-46bc-9a40-8faab20d9b15
begin
	data_plots_figure = Figure()
	
	# Full axis
	full_data_plot_axis = Axis(
		data_plots_figure[1,1],
		xlabel = L"t",
		ylabel = L"\Phi_{0,0}"
	)
	
	full_log_plot_axis = Axis(
		data_plots_figure[1,2],
		xlabel = L"t",
		ylabel = L"\log|\Phi_{0,0}|"
	)

	# Delimited axis
	delim_data_plot_axis = Axis(
		data_plots_figure[2,1],
		xlabel = L"t",
		ylabel = L"\Phi_{0,0}"
	)
	
	delim_log_plot_axis = Axis(
		data_plots_figure[2,2],
		xlabel = L"t",
		ylabel = L"\log|\Phi_{0,0}|"
	)
	
	# Full plots
	lines!(full_data_plot_axis, file_data."t", file_data."Re", color="black")
	lines!(full_log_plot_axis, file_data."t", log.(abs.(file_data."Re")), color="black")

	# Delimited plots
	lines!(
		delim_data_plot_axis, 
	...
	lines!(
		...
		file_data.t[delim_plot_start_idx:delim_plot_end_idx],
		log.(abs.(file_data.Re[delim_plot_start_idx:delim_plot_end_idx])),
...
qnm_model(t, p) = @. p[1] * exp.(-p[2] * t) * cos(p[3] * t -  p[4])
...
begin
	optimal_t0 = optimize(residual_at_idx, 66, 68).minimizer
...
begin
	...
	**$(qnm_fit.converged ? "Yes" : "No")**
	
	### Fit parameters:
	
	1. ``A`` = $(qnm_fit.param[1])
	2. ``2\omega_r`` = $(2*qnm_fit.param[3])
	3. ``2\omega_i`` = $(2*qnm_fit.param[2])
	4. ``\phi`` = $(qnm_fit.param[4])
...
begin
	fit_plots_figure = Figure()

	multipole_label = "\\Phi_{$(l),$(m)}"
	
	full_fit_plot_axis = Axis(
		fit_plots_figure[1,1],
		xlabel = L"t",
		ylabel = latexstring(multipole_label)
	)

	fit_residual_plot_axis = Axis(
		fit_plots_figure[1,2],
		xlabel = L"t",
		ylabel = "Fit residual"
	)
...
# ╔═╡ 357c595a-03fe-4fc9-9980-66ef220fe764
multipole_label

# ╔═╡ 29e47187-f569-4692-b038-a0d9609296a5
md"""
# Spectral analysis
"""

# ╔═╡ cfc53590-ad1a-4154-967e-4d60eb7a32a8
begin
	# Computing the periodogram
	data_dt = file_data.t[2] - file_data.t[1]
	data_fs = 1 / data_dt
	p = periodogram(file_data.Re, fs=data_fs)
	...
	lines!(field_axis, file_data.t, file_data.Re)
	lines!(psd_axis, p.freq, p.power)

	fig
end

# ╔═╡ e20d4ff2-5606-4ce3-80ef-3a96ca4b90bc
begin
	peak = maximum(p.power)
	peak_idx = findfirst(x -> x == maximum(p.power), p.power)
	
	md"""
	Peak power: $(peak)
	
	Peak frequency: $(p.freq[peak_idx])

	Peak omega: $(2 * pi * p.freq[peak_idx])
	"""
end
